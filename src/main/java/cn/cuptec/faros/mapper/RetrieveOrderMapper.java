...
public interface RetrieveOrderMapper extends BaseMapper<RetrieveOrder> {
    ...
    @Select("SELECT retrieve_order.*,patient_user.name as patientUserName, dept.name as deptName,retrieve_order_review_data.review_data as reviewData from retrieve_order " +
            "LEFT JOIN dept ON retrieve_order.dept_id = dept.id " +
